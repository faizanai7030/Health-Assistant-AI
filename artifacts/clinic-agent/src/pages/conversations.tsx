...
import { MessageSquare, Phone, User, Clock, ArrowLeft } from "lucide-react";
...
export default function Conversations() {
  ...
  return (
    <div className="h-[calc(100vh-10rem)] flex flex-col">
      <div className="mb-4">
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">WhatsApp Conversations</h1>
        <p className="text-muted-foreground mt-1 text-sm">Review AI agent interactions with patients.</p>
      </div>

      <div className="flex-1 overflow-hidden">

        {/* === MOBILE: full-width list → full-width thread === */}
        <div className="flex flex-col h-full md:hidden">
          {/* Mobile: Thread view */}
          {selectedId ? (
            <Card className="flex flex-col h-full overflow-hidden">
              <CardHeader className="py-3 px-4 border-b flex flex-row items-center gap-3 space-y-0">
                ...
                {isLoadingDetail ? (
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground flex items-center gap-1">
                      <Phone className="w-3 h-3" /> {detail.patientPhone}
                    </div>
                  </div>
                ) : null}
                {detail && (
                  <Badge variant={detail.status === 'active' ? "default" : "secondary"} className="ml-auto shrink-0">
                    {detail.status}
                  </Badge>
                )}
              </CardHeader>
                    ...
                    {conversations?.map(conv => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-1">
                          <div className="font-medium truncate pr-2">{conv.patientName || conv.patientPhone}</div>
                          {conv.lastMessageAt && (
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant={conv.status === 'active' ? "default" : "secondary"} className="text-[10px] h-5">
                            {conv.status}
                          </Badge>
                        </div>
                      </div>
