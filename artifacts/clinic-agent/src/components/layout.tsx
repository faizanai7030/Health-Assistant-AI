...
import { LayoutDashboard, Users, Calendar, MessageSquare, MessageCircleCode, Bell, LogOut, Settings, ClipboardList, BookOpen } from "lucide-react";
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { href: "/onboarding-guide", label: "Onboarding Guide", icon: BookOpen },
    ...
    <div className="flex h-screen bg-background w-full">
        ...
        <div className="p-6 border-b flex items-center space-x-3">
          ...
          <div className="min-w-0">
            <div className="font-bold text-sm tracking-tight text-foreground truncate">
              {user?.clinicName ?? "A.I'll Handle It"}
            </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden w-full">
        ...
        <header className="md:hidden border-b bg-card p-4 flex items-center justify-between">
          <div className="font-bold text-lg text-foreground">{user?.clinicName ?? "A.I'll Handle It"}</div>
